 ...
 *Created by Martin.Cheng@rock-chips.com
 *thanks no name author for old version
 *date: 2014-12-31
 */
...
public class BytesEncodingDetector extends Encoding {
  private final String TAG = "BytesEncodingDetector";
   ...
   */
  public String detectEncoding(URL testurl) {
    byte[] rawBytes = new byte[10000];
    int bytesread = 0, byteoffset = 0;
    String encodeType = null;
    InputStream chinesestream;
    try {
      chinesestream = testurl.openStream();
      while ((bytesread = chinesestream.read(rawBytes, byteoffset, rawBytes.length - byteoffset)) > 0) {
        byteoffset += bytesread;
      }
      ;
      chinesestream.close();
      encodeType = detectEncoding(rawBytes);
    } catch (Exception e) {
   ...
   */
  public String detectEncoding(File inFile) {
    FileInputStream inStream;
    byte[] rawBytes;
    rawBytes = new byte[(int) inFile.length()];
    try {
    ...
    return detectEncoding(rawBytes);
  }
   ...
   */
  public String detectEncoding(byte[] rawBytes) {
    int[] scores;
    int index, maxscore = 0;
    int encoding_guess = OTHER;
    scores = new int[TOTALTYPES];
    // Assign Scores
    scores[GB2312] = evaluateGB2312(rawBytes);
    scores[GBK] = evaluateGBK(rawBytes);
    scores[GB18030] = evaluateGB18030(rawBytes);
    scores[HZ] = evaluateHZ(rawBytes);
    scores[BIG5] = evaluateBIG5(rawBytes);
    scores[CNS11643] = evaluateEucTW(rawBytes);
    scores[UTF8] = evaluateUTF8(rawBytes);
	scores[UTF8_BE] = 0;
	scores[UTF8_LE] = 0;
    scores[UNICODE] = evaluateUTF16(rawBytes);
    scores[UNICODE_BE] = evaluateUTF16BigEndian(rawBytes);
    scores[UNICODE_LE] = evaluateUTF16LittleEndian(rawBytes);
	scores[ISO2022CN] = evaluateISO2022cn(rawBytes);
	scores[ISO2022CN_CNS] = 0;
	scores[ISO2022CN_GB] = 0;

    scores[EUC_KR] = evaluateEucKR(rawBytes);
    scores[CP949] = evaluateCP949(rawBytes);
    scores[JOHAB] = 0;
    scores[ISO2022KR] = evaluateISO2022KR(rawBytes);
    scores[ASCII] = evaluateASCII(rawBytes);
    scores[SJIS] = evaluateSJIS(rawBytes);
    scores[EUC_JP] = evaluateEucJP(rawBytes);
    scores[ISO2022JP] = evaluateISO2022JP(rawBytes);
    scores[OTHER] = 0;
    // Tabulate Scores
    for (index = 0; index < TOTALTYPES; index++) {
      //LogUtil.LogPlayer(TAG,"Encoding " + mCharsetJava[index] + " score " + scores[index]);
	...
	return mCharsetJava[encoding_guess];
  }

  /*
   * Function: evaluateGB2312 Argument: pointer to byte array Returns : number from 0 to 100 representing probability text
   * in array uses GB-2312 encoding
   */
  private int evaluateGB2312(byte[] rawBytes) {
    int i, rawLen = 0;
    ...
    for (i = 0; i < rawLen - 1; i++) {
      // System.err.println(rawBytes[i]);
      if (rawBytes[i] >= 0) {
        // asciichars++;
      } else {
        dbchars++;
        if ((byte) 0xA1 <= rawBytes[i] && rawBytes[i] <= (byte) 0xF7 && (byte) 0xA1 <= rawBytes[i + 1]
            && rawBytes[i + 1] <= (byte) 0xFE) {
          gbchars++;
          totalfreq += 500;
          row = rawBytes[i] + 256 - 0xA1;
          column = rawBytes[i + 1] + 256 - 0xA1;
  ...
  /*
   * Function: evaluateGBK Argument: pointer to byte array Returns : number from 0 to 100 representing probability text in
   * array uses GBK encoding
   */
  private int evaluateGBK(byte[] rawBytes) {
    int i, rawLen = 0;
    int dbchars = 1, gbchars = 1;
    long gbfreq = 0, totalfreq = 1;
    float rangeval = 0, freqval = 0;
    ...
    for (i = 0; i < rawLen - 1; i++) {
      ...
      if (rawBytes[i] >= 0) {
        ...
        dbchars++;
        if ((byte) 0xA1 <= rawBytes[i] && rawBytes[i] <= (byte) 0xF7 && // Original GB range
            (byte) 0xA1 <= rawBytes[i + 1] && rawBytes[i + 1] <= (byte) 0xFE) {
          gbchars++;
          totalfreq += 500;
          row = rawBytes[i] + 256 - 0xA1;
          column = rawBytes[i + 1] + 256 - 0xA1;
        ...
        } else if ((byte) 0x81 <= rawBytes[i]
            && rawBytes[i] <= (byte) 0xFE
            && // Extended GB range
            (((byte) 0x80 <= rawBytes[i + 1] && rawBytes[i + 1] <= (byte) 0xFE) || ((byte) 0x40 <= rawBytes[i + 1] && rawBytes[i + 1] <= (byte) 0x7E))) {
          gbchars++;
          totalfreq += 500;
          row = rawBytes[i] + 256 - 0x81;
          if (0x40 <= rawBytes[i + 1] && rawBytes[i + 1] <= 0x7E) {
            column = rawBytes[i + 1] - 0x40;
          } else {
            column = rawBytes[i + 1] + 256 - 0x40;
          }
          // System.out.println("extended row " + row + " column " + column + " rawBytes[i] " + rawBytes[i]);
  ...
  /*
   * Function: evaluateGB18030 Argument: pointer to byte array Returns : number from 0 to 100 representing probability text
   * in array uses GBK encoding
   */
  private int evaluateGB18030(byte[] rawBytes) {
    int i, rawLen = 0;
    int dbchars = 1, gbchars = 1;
    long gbfreq = 0, totalfreq = 1;
    float rangeval = 0, freqval = 0;
    ...
    for (i = 0; i < rawLen - 1; i++) {
      ...
      if (rawBytes[i] >= 0) {
        ...
        dbchars++;
        if ((byte) 0xA1 <= rawBytes[i] && rawBytes[i] <= (byte) 0xF7 && // Original GB range
            i + 1 < rawLen && (byte) 0xA1 <= rawBytes[i + 1] && rawBytes[i + 1] <= (byte) 0xFE) {
          gbchars++;
          totalfreq += 500;
          row = rawBytes[i] + 256 - 0xA1;
          column = rawBytes[i + 1] + 256 - 0xA1;
        ...
        } else if ((byte) 0x81 <= rawBytes[i] && rawBytes[i] <= (byte) 0xFE
            && // Extended GB range
            i + 1 < rawLen
            && (((byte) 0x80 <= rawBytes[i + 1] && rawBytes[i + 1] <= (byte) 0xFE) || ((byte) 0x40 <= rawBytes[i + 1] && rawBytes[i + 1] <= (byte) 0x7E))) {
          gbchars++;
          ...
          if (GBKFreq[row][column] != 0) {
            ...
            && rawBytes[i] <= (byte) 0xFE
            && // Extended GB range
            i + 3 < rawLen && (byte) 0x30 <= rawBytes[i + 1] && rawBytes[i + 1] <= (byte) 0x39
            && (byte) 0x81 <= rawBytes[i + 2] && rawBytes[i + 2] <= (byte) 0xFE && (byte) 0x30 <= rawBytes[i + 3]
            && rawBytes[i + 3] <= (byte) 0x39) {
          gbchars++;
        }
        i++;
      }
  ...
  /*
   * Function: evaluateHZ Argument: byte array Returns : number from 0 to 100 representing probability text in array uses HZ
   * encoding
   */
  private int evaluateHZ(byte[] rawBytes) {
    int i, rawLen;
    ...
    for (i = 0; i < rawLen; i++) {
      if (rawBytes[i] == '~') {
        if (rawBytes[i + 1] == '{') {
          ...
          i += 2;
          while (i < rawLen - 1) {
            if (rawBytes[i] == 0x0A || rawBytes[i] == 0x0D) {
              break;
            } else if (rawBytes[i] == '~' && rawBytes[i + 1] == '}') {
              hzend++;
              i++;
              break;
            } else if ((0x21 <= rawBytes[i] && rawBytes[i] <= 0x77) && (0x21 <= rawBytes[i + 1] && rawBytes[i + 1] <= 0x77)) {
              hzchars += 2;
              row = rawBytes[i] - 0x21;
              column = rawBytes[i + 1] - 0x21;
              totalfreq += 500;
              if (GBFreq[row][column] != 0) {
            ...
            } else if ((0xA1 <= rawBytes[i] && rawBytes[i] <= 0xF7) && (0xA1 <= rawBytes[i + 1] && rawBytes[i + 1] <= 0xF7)) {
              hzchars += 2;
              row = rawBytes[i] + 256 - 0xA1;
              column = rawBytes[i + 1] + 256 - 0xA1;
              totalfreq += 500;
  ...
  /**
   * Function: evaluateBIG5 Argument: byte array Returns : number from 0 to 100 representing probability text in array uses
   * Big5 encoding
   */
  private int evaluateBIG5(byte[] rawBytes) {
    int i, rawLen = 0;
    ...
    for (i = 0; i < rawLen - 1; i++) {
      if (rawBytes[i] >= 0) {
        ...
        dbchars++;
        if ((byte) 0xA1 <= rawBytes[i]
            && rawBytes[i] <= (byte) 0xF9
            && (((byte) 0x40 <= rawBytes[i + 1] && rawBytes[i + 1] <= (byte) 0x7E) || ((byte) 0xA1 <= rawBytes[i + 1] && rawBytes[i + 1] <= (byte) 0xFE))) {
          bfchars++;
          totalfreq += 500;
          row = rawBytes[i] + 256 - 0xA1;
          if (0x40 <= rawBytes[i + 1] && rawBytes[i + 1] <= 0x7E) {
  ...
  /*
   * Function: evaluateBIG5Plus Argument: pointer to unsigned char array Returns : number from 0 to 100 representing
   * probability text in array uses Big5+ encoding
   */
  private int evaluateBIG5Plus(byte[] rawBytes) {
    int i, rawLen = 0;
    int dbchars = 1, bfchars = 1;
    long bffreq = 0, totalfreq = 1;
    float rangeval = 0, freqval = 0;
    ...
    for (i = 0; i < rawLen - 1; i++) {
      ...
      if (rawBytes[i] >= 128) {
        ...
        dbchars++;
        if (0xA1 <= rawBytes[i] && rawBytes[i] <= 0xF9 && // Original Big5 range
            ((0x40 <= rawBytes[i + 1] && rawBytes[i + 1] <= 0x7E) || (0xA1 <= rawBytes[i + 1] && rawBytes[i + 1] <= 0xFE))) {
          bfchars++;
          totalfreq += 500;
          row = rawBytes[i] - 0xA1;
          if (0x40 <= rawBytes[i + 1] && rawBytes[i + 1] <= 0x7E) {
        ...
        } else if (0x81 <= rawBytes[i] && rawBytes[i] <= 0xFE && // Extended Big5 range
            ((0x40 <= rawBytes[i + 1] && rawBytes[i + 1] <= 0x7E) || (0x80 <= rawBytes[i + 1] && rawBytes[i + 1] <= 0xFE))) {
          bfchars++;
          totalfreq += 500;
          row = rawBytes[i] - 0x81;
          if (0x40 <= rawBytes[i + 1] && rawBytes[i + 1] <= 0x7E) {
  ...
  /*
   * Function: evaluateEucTW Argument: byte array Returns : number from 0 to 100 representing probability text in array
   * uses EUC-TW (CNS 11643) encoding
   */
  private int evaluateEucTW(byte[] rawBytes) {
    int i, rawLen = 0;
    ...
    rawLen = rawBytes.length;
    for (i = 0; i < rawLen - 1; i++) {
      if (rawBytes[i] >= 0) { // in ASCII range
        // asciichars++;
      } else { // high bit set
        dbchars++;
        if (i + 3 < rawLen && (byte) 0x8E == rawBytes[i] && (byte) 0xA1 <= rawBytes[i + 1] && rawBytes[i + 1] <= (byte) 0xB0
            && (byte) 0xA1 <= rawBytes[i + 2] && rawBytes[i + 2] <= (byte) 0xFE && (byte) 0xA1 <= rawBytes[i + 3]
            && rawBytes[i + 3] <= (byte) 0xFE) { // Planes 1 - 16
          cnschars++;
          // System.out.println("plane 2 or above CNS char");
          // These are all less frequent chars so just ignore freq
          i += 3;
        } else if ((byte) 0xA1 <= rawBytes[i] && rawBytes[i] <= (byte) 0xFE && // Plane 1
            (byte) 0xA1 <= rawBytes[i + 1] && rawBytes[i + 1] <= (byte) 0xFE) {
          cnschars++;
          totalfreq += 500;
          row = rawBytes[i] + 256 - 0xA1;
  ...
  /*
   * Function: evaluateISO2022cn Argument: byte array Returns : number from 0 to 100 representing probability text in
   * array uses ISO 2022-CN encoding WORKS FOR BASIC CASES, BUT STILL NEEDS MORE WORK
   */
  private int evaluateISO2022cn(byte[] rawBytes) {
    int i, rawLen = 0;
    ...
    rawLen = rawBytes.length;
    for (i = 0; i < rawLen - 1; i++) {
      if (rawBytes[i] == (byte) 0x1B && i + 3 < rawLen) { // Escape char ESC
        if (rawBytes[i + 1] == (byte) 0x24 && rawBytes[i + 2] == 0x29 && rawBytes[i + 3] == (byte) 0x41) { // GB Escape $ ) A
          i += 4;
          while (rawBytes[i] != (byte) 0x1B) {
            dbchars++;
            if ((0x21 <= rawBytes[i] && rawBytes[i] <= 0x77) && (0x21 <= rawBytes[i + 1] && rawBytes[i + 1] <= 0x77)) {
              isochars++;
              row = rawBytes[i] - 0x21;
              column = rawBytes[i + 1] - 0x21;
              totalfreq += 500;
        ...
        } else if (i + 3 < rawLen && rawBytes[i + 1] == (byte) 0x24 && rawBytes[i + 2] == (byte) 0x29
            && rawBytes[i + 3] == (byte) 0x47) {
          // CNS Escape $ ) G
          i += 4;
          while (rawBytes[i] != (byte) 0x1B) {
            dbchars++;
            if ((byte) 0x21 <= rawBytes[i] && rawBytes[i] <= (byte) 0x7E && (byte) 0x21 <= rawBytes[i + 1]
                && rawBytes[i + 1] <= (byte) 0x7E) {
              isochars++;
              totalfreq += 500;
              row = rawBytes[i] - 0x21;
              column = rawBytes[i + 1] - 0x21;
        ...
        if (rawBytes[i] == (byte) 0x1B && i + 2 < rawLen && rawBytes[i + 1] == (byte) 0x28 && rawBytes[i + 2] == (byte) 0x42) { // ASCII:
          // ESC
          // ( B
          i += 2;
        }
  ...
  /*
   * Function: evaluateUTF8 Argument: byte array Returns : number from 0 to 100 representing probability text in array uses
   * UTF-8 encoding of Unicode
   */
  private int evaluateUTF8(byte[] rawBytes) {
    int score = 0;
    int i, rawLen = 0;
    ...
    rawLen = rawBytes.length;
    for (i = 0; i < rawLen; i++) {
      if ((rawBytes[i] & (byte) 0x7F) == rawBytes[i]) { // One byte
        asciibytes++;
        // Ignore ASCII, can throw off count
      } else if (-64 <= rawBytes[i] && rawBytes[i] <= -33 && // Two bytes
          i + 1 < rawLen && -128 <= rawBytes[i + 1] && rawBytes[i + 1] <= -65) {
        goodbytes += 2;
        i++;
      } else if (-32 <= rawBytes[i] && rawBytes[i] <= -17
          && // Three bytes
          i + 2 < rawLen && -128 <= rawBytes[i + 1] && rawBytes[i + 1] <= -65 && -128 <= rawBytes[i + 2]
          && rawBytes[i + 2] <= -65) {
        goodbytes += 3;
        i += 2;
      }
    }
    if (asciibytes == rawLen) {
      return 0;
    }
    score = (int) (100 * ((float) goodbytes / (float) (rawLen - asciibytes)));
    // System.out.println("rawLen " + rawLen + " goodbytes " + goodbytes + " asciibytes " + asciibytes + " score " +
    // score);
  ...
  private int evaluateUTF16BigEndian(byte[] rawBytes) {
    if(rawBytes.length > 1 && ((byte) 0xFE == rawBytes[0] && (byte) 0xFF == rawBytes[1])){
		return 100;
    }
    return 0;
  }

  // detect little endian unicode
  private int evaluateUTF16LittleEndian(byte[] rawBytes) {
    if(rawBytes.length > 1 && ((byte) 0xFF == rawBytes[0] && (byte) 0xFE == rawBytes[1])){
		return 100;
    }
    return 0;
  }
  ...
  /*
   * Function: evaluateUTF16 Argument: byte array Returns : number from 0 to 100 representing probability text in array uses
   * UTF-16 encoding of Unicode, guess based on BOM // NOT VERY GENERAL, NEEDS MUCH MORE WORK
   */
  private int evaluateUTF16(byte[] rawBytes) {
    if (rawBytes.length > 1 && ((byte) 0xFE == rawBytes[0] && (byte) 0xFF == rawBytes[1]) || // Big-endian
        ((byte) 0xFF == rawBytes[0] && (byte) 0xFE == rawBytes[1])) { // Little-endian
      return 80;
    }
  ...
  /*
   * Function: evaluateASCII Argument: byte array Returns : number from 0 to 100 representing probability text in array uses
   * all ASCII Description: Sees if array has any characters not in ASCII range, if so, score is reduced
   */
  private int evaluateASCII(byte[] rawBytes) {
    int score = 75;
    int i, rawLen;
    rawLen = rawBytes.length;
    for (i = 0; i < rawLen; i++) {
      if (rawBytes[i] < 0) {
        score = score - 5;
      } else if (rawBytes[i] == (byte) 0x1B) { // ESC (used by ISO 2022)
        score = score - 5;
      }
      if (score <= 0) {
        return 0;
   ...
   */
  private int evaluateEucKR(byte[] rawBytes) {
    int i, rawLen = 0;
    ...
    for (i = 0; i < rawLen - 1; i++) {
      ...
      if (rawBytes[i] >= 0) {
        ...
        dbchars++;
        if ((byte) 0xA1 <= rawBytes[i] && rawBytes[i] <= (byte) 0xFE && (byte) 0xA1 <= rawBytes[i + 1]
            && rawBytes[i + 1] <= (byte) 0xFE) {
          krchars++;
          totalfreq += 500;
          row = rawBytes[i] + 256 - 0xA1;
   ...
   */
  private int evaluateCP949(byte[] rawBytes) {
    int i, rawLen = 0;
    int dbchars = 1, krchars = 1;
    long krfreq = 0, totalfreq = 1;
    float rangeval = 0, freqval = 0;
    ...
    for (i = 0; i < rawLen - 1; i++) {
      ...
      if (rawBytes[i] >= 0) {
        ...
        dbchars++;
        if ((byte) 0x81 <= rawBytes[i]
            && rawBytes[i] <= (byte) 0xFE
            && ((byte) 0x41 <= rawBytes[i + 1] && rawBytes[i + 1] <= (byte) 0x5A || (byte) 0x61 <= rawBytes[i + 1]
                && rawBytes[i + 1] <= (byte) 0x7A || (byte) 0x81 <= rawBytes[i + 1] && rawBytes[i + 1] <= (byte) 0xFE)) {
          krchars++;
          totalfreq += 500;
          if ((byte) 0xA1 <= rawBytes[i] && rawBytes[i] <= (byte) 0xFE && (byte) 0xA1 <= rawBytes[i + 1]
              && rawBytes[i + 1] <= (byte) 0xFE) {
  ...
  private int evaluateISO2022KR(byte[] rawBytes) {
    int i;
    for (i = 0; i < rawBytes.length; i++) {
      if (i + 3 < rawBytes.length && rawBytes[i] == 0x1b && (char) rawBytes[i + 1] == '$' && (char) rawBytes[i + 2] == ')'
          && (char) rawBytes[i + 3] == 'C') {
        return 100;
      }
    }
  ...
  /*
   * Function: evaluateEucJP Argument: pointer to byte array Returns : number from 0 to 100 representing probability text
   * in array uses EUC-JP encoding
   */
  private int evaluateEucJP(byte[] rawBytes) {
    int i, rawLen = 0;
  ...
  private int evaluateISO2022JP(byte[] rawBytes) {
    int i;
    for (i = 0; i < rawBytes.length; i++) {
      if (i + 2 < rawBytes.length && rawBytes[i] == 0x1b && (char) rawBytes[i + 1] == '$' && (char) rawBytes[i + 2] == 'B') {
        return 100;
      }
    }
    return 0;
  ...
  /*
   * Function: evaluateSJIS Argument: pointer to byte array Returns : number from 0 to 100 representing probability text in
   * array uses Shift-JIS encoding
   */
  private int evaluateSJIS(byte[] rawBytes) {
    int i, rawLen = 0;
    int dbchars = 1, jpchars = 1;
    long jpfreq = 0, totalfreq = 1;
    float rangeval = 0, freqval = 0;
    ...
    for (i = 0; i < rawLen - 1; i++) {
      ...
      if (rawBytes[i] >= 0) {
        ...
        if (i + 1 < rawBytes.length
            && (((byte) 0x81 <= rawBytes[i] && rawBytes[i] <= (byte) 0x9F) || ((byte) 0xE0 <= rawBytes[i] && rawBytes[i] <= (byte) 0xEF))
            && (((byte) 0x40 <= rawBytes[i + 1] && rawBytes[i + 1] <= (byte) 0x7E) || ((byte) 0x80 <= rawBytes[i + 1] && rawBytes[i + 1] <= (byte) 0xFC))) {
          jpchars++;
          totalfreq += 500;
          row = rawBytes[i] + 256;
          column = rawBytes[i + 1] + 256;
          if (column < 0x9f) {
            adjust = 1;
            if (column > 0x7f) {
              column -= 0x20;
          ...
          i++;
        } else if ((byte) 0xA1 <= rawBytes[i] && rawBytes[i] <= (byte) 0xDF) {
  ...
  private void initEncodingFrequencies() {
    int i, j;
    for (i = 0; i < 94; i++) {
      for (j = 0; j < 94; j++) {
        GBFreq[i][j] = 0;
...
class Encoding {
  ...
  public static int UTF8_BE = 7;

  public static int UTF8_LE = 8;

  public static int UNICODE = 9;

  public static int UNICODE_BE = 10;

  public static int UNICODE_LE = 11;
  ...
  public static String[] mCharsetJava;

  // Names of the encodings for human viewing
  public static String[] mCharsetNice;

  // Names of charsets as used in charset parameter of HTML Meta tag
  public static String[] mCharsetHTML;

  // Constructor
  public Encoding() {
    mCharsetJava = new String[TOTALTYPES];
    mCharsetNice = new String[TOTALTYPES];
    mCharsetHTML = new String[TOTALTYPES];
    // Assign encoding names
    mCharsetJava[GB2312] = "GB2312";
    mCharsetJava[GBK] = "GBK";
    mCharsetJava[GB18030] = "GB18030";
    mCharsetJava[HZ] = "ASCII"; // What to put here? Sun doesn't support HZ
    mCharsetJava[ISO2022CN_GB] = "ISO2022CN_GB";
    mCharsetJava[BIG5] = "BIG5";
    mCharsetJava[CNS11643] = "EUC-TW";
    mCharsetJava[ISO2022CN_CNS] = "ISO2022CN_CNS";
    mCharsetJava[ISO2022CN] = "ISO2022CN";
    mCharsetJava[UTF8] = "UTF8";
    mCharsetJava[UTF8_BE] = "UTF8";
    mCharsetJava[UTF8_LE] = "UTF8";
    mCharsetJava[UNICODE] = "Unicode";
    mCharsetJava[UNICODE_BE] = "Unicode";
    mCharsetJava[UNICODE_LE] = "Unicode";
    mCharsetJava[EUC_KR] = "EUC_KR";
    mCharsetJava[CP949] = "MS949";
    mCharsetJava[ISO2022KR] = "ISO2022KR";
    mCharsetJava[JOHAB] = "Johab";
    mCharsetJava[SJIS] = "SJIS";
    mCharsetJava[EUC_JP] = "EUC_JP";
    mCharsetJava[ISO2022JP] = "ISO2022JP";
    mCharsetJava[ASCII] = "ASCII";
    mCharsetJava[OTHER] = "ISO8859_1";
    // Assign encoding names
    mCharsetHTML[GB2312] = "GB2312";
    mCharsetHTML[GBK] = "GBK";
    mCharsetHTML[GB18030] = "GB18030";
    mCharsetHTML[HZ] = "HZ-GB-2312";
    mCharsetHTML[ISO2022CN_GB] = "ISO-2022-CN-EXT";
    mCharsetHTML[BIG5] = "BIG5";
    mCharsetHTML[CNS11643] = "EUC-TW";
    mCharsetHTML[ISO2022CN_CNS] = "ISO-2022-CN-EXT";
    mCharsetHTML[ISO2022CN] = "ISO-2022-CN";
    mCharsetHTML[UTF8] = "UTF-8";
    mCharsetHTML[UTF8_BE] = "UTF-8";
    mCharsetHTML[UTF8_LE] = "UTF-8";
    mCharsetHTML[UNICODE] = "UTF-16";
    mCharsetHTML[UNICODE_BE] = "UTF-16";
    mCharsetHTML[UNICODE_LE] = "UTF-16";
    mCharsetHTML[EUC_KR] = "EUC-KR";
    mCharsetHTML[CP949] = "x-windows-949";
    mCharsetHTML[ISO2022KR] = "ISO-2022-KR";
    mCharsetHTML[JOHAB] = "x-Johab";
    mCharsetHTML[SJIS] = "Shift_JIS";
    mCharsetHTML[EUC_JP] = "EUC-JP";
    mCharsetHTML[ISO2022JP] = "ISO-2022-JP";
    mCharsetHTML[ASCII] = "ASCII";
    mCharsetHTML[OTHER] = "ISO8859-1";
    // Assign Human readable names
    mCharsetNice[GB2312] = "GB-2312";
    mCharsetNice[GBK] = "GBK";
    mCharsetNice[GB18030] = "GB18030";
    mCharsetNice[HZ] = "HZ";
    mCharsetNice[ISO2022CN_GB] = "ISO2022CN-GB";
    mCharsetNice[BIG5] = "Big5";
    mCharsetNice[CNS11643] = "CNS11643";
    mCharsetNice[ISO2022CN_CNS] = "ISO2022CN-CNS";
    mCharsetNice[ISO2022CN] = "ISO2022 CN";
    mCharsetNice[UTF8] = "UTF-8";
    mCharsetNice[UTF8_BE] = "UTF-BigEndian";
    mCharsetNice[UTF8_LE] = "UTF-LittleEndian";
    mCharsetNice[UNICODE] = "Unicode";
    mCharsetNice[UNICODE_BE] = "Unicode-BigEndian";
    mCharsetNice[UNICODE_LE] = "Unicode-LittleEndian";
    mCharsetNice[EUC_KR] = "EUC-KR";
    mCharsetNice[CP949] = "CP949";
    mCharsetNice[ISO2022KR] = "ISO 2022 KR";
    mCharsetNice[JOHAB] = "Johab";
    mCharsetNice[SJIS] = "Shift-JIS";
    mCharsetNice[EUC_JP] = "EUC-JP";
    mCharsetNice[ISO2022JP] = "ISO 2022 JP";
    mCharsetNice[ASCII] = "ASCII";
    mCharsetNice[OTHER] = "OTHER";
  }
}
