...
public class DoubleColumnMenu extends RelativeLayout implements Observer {
	...
	private void initAttributes(Context context, AttributeSet attrs) {
		...
		mMenuFocusBg = styledAttrs.getResourceId(R.styleable.DoubleColumnMenu_menuFocusResId,
				android.R.color.transparent);
		mMenuFocusMode = styledAttrs.getInt(R.styleable.DoubleColumnMenu_menuFocusMode, FOCUS_MODE_TMP);

		mSubMenuWidth = styledAttrs.getDimensionPixelSize(R.styleable.DoubleColumnMenu_subMenuWidth, 300);
		mSubMenuBg = styledAttrs.getResourceId(R.styleable.DoubleColumnMenu_subMenuBackground,
				android.R.color.transparent);
		mSubMenuFocusBg = styledAttrs.getResourceId(R.styleable.DoubleColumnMenu_subMenuFocusResId,
				android.R.color.transparent);
		mSubMenuFocusMode = styledAttrs.getInt(R.styleable.DoubleColumnMenu_subMenuFocusMode, FOCUS_MODE_TMP);
	...
	private void setupLayout(Context context) {
		...
		RelativeLayout.LayoutParams subMenuRootLp = new RelativeLayout.LayoutParams(
		...
		FrameLayout.LayoutParams subMenuLp = new FrameLayout.LayoutParams(RelativeLayout.LayoutParams.WRAP_CONTENT,
		...
		RelativeLayout.LayoutParams menuRootLp = new RelativeLayout.LayoutParams(
				RelativeLayout.LayoutParams.WRAP_CONTENT, RelativeLayout.LayoutParams.MATCH_PARENT);
		...
		mMenuContainer.setOnFocusChangeListener(new OnFocusChangeListener() {
			...
			public void onFocusChange(View v, boolean hasFocus) {
				if (hasFocus) {
					...
					if (mMenuFocusMode == FOCUS_MODE_SELECTED && mMenuSelectedPosi != POSITION_NULL
							&& mMenuSelectedPosi < childCount) {
		...
		ScrollView.LayoutParams menuContainerLp = new ScrollView.LayoutParams(ScrollView.LayoutParams.MATCH_PARENT,
		...
		RelativeLayout.LayoutParams subMenuBgLp = new RelativeLayout.LayoutParams(mSubMenuWidth,
				RelativeLayout.LayoutParams.MATCH_PARENT);
		...
		RelativeLayout.LayoutParams subMenuScrollViewLp = new RelativeLayout.LayoutParams(mSubMenuWidth,
				FrameLayout.LayoutParams.MATCH_PARENT);
		...
		mSubMenuContainer.setOnFocusChangeListener(new OnFocusChangeListener() {
			...
			public void onFocusChange(View v, boolean hasFocus) {
				if (hasFocus) {
					int childCount = ((LinearLayout) v).getChildCount();
					int posi = POSITION_NULL;
					if (mSubMenuFocusMode == FOCUS_MODE_SELECTED) {
		...
		ScrollView.LayoutParams subMenuContainerLp = new ScrollView.LayoutParams(ScrollView.LayoutParams.MATCH_PARENT,
				ScrollView.LayoutParams.WRAP_CONTENT);
	...
	public void scrollMenu2Posi(int position) {
		if (position >= mMenuContainer.getChildCount()) {
			StringBuilder sb = new StringBuilder("Fail to scrollMenu2Posi. [There are total ")
					.append(mMenuContainer.getChildCount()).append(" child in menu, but your choice of position is ")
	...
	public void scrollSubMenu2Posi(int position) {
		if (position >= mSubMenuContainer.getChildCount()) {
			StringBuilder sb = new StringBuilder("Fail to scrollSubMenu2Posi. [There are total ")
					.append(mSubMenuContainer.getChildCount())
					.append(" child in right List, but your choice of position is ").append(position).append("]");
			Log.w(TAG, sb.toString());
			return;
		}
		...
		for (int i = 0; i < childCount; i++) {
			...
			child.setOnKeyListener(new OnKeyListener() {
				...
				public boolean onKey(View v, int keyCode, KeyEvent event) {
					if (mOnKeyListener != null && mOnKeyListener.onMenuItemKeyEvent(v, keyCode, event)) {
						return true;
					}
	...
	private void configSubMenuItem(View newView, final int position, int totalCount) {
		if (newView instanceof ViewGroup) {
			ViewGroup viewGroup = (ViewGroup) newView;
			viewGroup.setDescendantFocusability(ViewGroup.FOCUS_BLOCK_DESCENDANTS);
		}
		...
		newView.setOnFocusChangeListener(new OnFocusChangeListener() {
			...
			public void onFocusChange(final View v, boolean hasFocus) {
				if (hasFocus) {
					...
					v.startAnimation(mZoomOutAnim);
				}
				if (mFocusChangeListener != null) {
					mFocusChangeListener.onSubMenuItemFocusChanged(mSubMenuContainer, v, position, hasFocus);
				}
			}
		});
	}
	...
	private void setupSubMenuItemKeyEvent() {
		...
		for (int i = 0; i < childCount; i++) {
			...
			child.setOnKeyListener(new OnKeyListener() {
				...
				@Override
				public boolean onKey(View v, int keyCode, KeyEvent event) {
					if (mOnKeyListener != null && mOnKeyListener.onSubMenuItemKeyEvent(v, keyCode, event)) {
						return true;
					}
