...
public class MediaListItemView extends MediaItemView {
	...
	public MediaListItemView(Context context, AttributeSet attrs, int defStyle) {
		...
		int relativeLayoutWidth = (int) getResources().getDimension(
				R.dimen.px1600);
		int relativeLayoutHeight = (int) getResources().getDimension(
				R.dimen.px200);
		RelativeLayout.LayoutParams relativeLayoutParams = new RelativeLayout.LayoutParams(
				relativeLayoutWidth, relativeLayoutHeight);
		relativeLayout.setLayoutParams(relativeLayoutParams);

		mContext = context;
		mPlayFocusDrawable = getResources().getDrawable(
				R.drawable.videoplaynomal);
		mNumDrawable = new NumberDrawable(context);
		setWillNotDraw(false);
		setFocusable(false);

		mFocusView = new ImageView(context);
		LayoutParams mFocusParams = new RelativeLayout.LayoutParams(
		...
		LayoutParams mediaParams = new RelativeLayout.LayoutParams(
				(int) getResources().getDimension(R.dimen.px200),
				(int) getResources().getDimension(R.dimen.px168));
...
//				FileUtil.dip2px(mContext, 42), 0, 0);
		...
		mTvName.setTextColor(getResources().getColorStateList(
				R.color.btn_selector));
		mTvName.setTextSize(getResources().getDimension(R.dimen.px24));
		mTvName.setSingleLine(true);
		LayoutParams tvParams = new RelativeLayout.LayoutParams(
		...
		mTvSize.setTextColor(getResources().getColorStateList(
				R.color.list_item_font_selector));
		mTvSize.setTextSize(getResources().getDimension(R.dimen.px18));
		LayoutParams tvSizeParams = new RelativeLayout.LayoutParams(
				LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
		...
		mTvDate.setTextColor(getResources().getColorStateList(
				R.color.list_item_font_selector));
		LayoutParams tvDateParams = new RelativeLayout.LayoutParams(
				LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
	...
	public void setMediaItem(Media media) {
		...
		if ((mMedia.mType == SourceType.FOLDER)
				&& !usbRootPaths.contains(mMedia.mUri)) {
		...
		mTvDate.setText("日期: "
				+ DateUtil.onDate2String(new Date(media.modifiedDate),
						"yyyy.MM.dd HH:mm"));
