...
public class LyricView extends View {
	...
	public LyricView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		mPaint.setTextAlign(mTextAlign == TEXT_ALIGN_CENTER ? Align.CENTER
				: (mTextAlign == TEXT_ALIGN_LEFT ? Align.LEFT : Align.RIGHT));
	...
	protected void onDraw(Canvas canvas) {
		...
		drawDistanceY = 0;
		for (int postLine = mFocusedLineIndex + 1, lineCount = mLyricInfo.getLyrics()
				.size(); postLine < lineCount; postLine++) {
			...
			lineY = mFocusLineY + (postLine - mFocusedLineIndex - 1) * (mLineSpacing + mTextHeight) + mLineSpacing
					+ mFocusTextHeight - textAscent + offset;
	...
	public void setCurrTime(final long timeInMillis) {
		...
		if (mDestLineIndex > startOffsetLineIndex) {
			if (mDestLineIndex > mFocusedLineIndex) {
				...
				postFocusOffsetY = -(mDestLineIndex - mFocusedLineIndex - 1) * (mLineSpacing + mTextHeight)
						- mFocusTextHeight - mLineSpacing;
		...
		if (preFocusoffsetY != 0 || postFocusOffsetY != 0) {
			mGradientInfo.textColor = mTextColor;
			mGradientInfo.focusTextColor = mFocusTextColor;
			GradientInfo endInfo = new GradientInfo(preFocusoffsetY, postFocusOffsetY, mFocusTextSize - mTextSize,
					mFocusTextColor, mTextColor);
	...
	public class OffsetEvaluator implements TypeEvaluator<GradientInfo> {
		...
		public GradientInfo evaluate(float fraction, GradientInfo startValue, GradientInfo endValue) {
			startValue.preFocusOffsetY = startValue.preFocusOffsetY
					+ (endValue.preFocusOffsetY - startValue.preFocusOffsetY) * fraction;
			startValue.postFocusOffsetY = startValue.postFocusOffsetY
					+ (endValue.postFocusOffsetY - startValue.postFocusOffsetY) * fraction;
			startValue.textSizeOffset = startValue.textSizeOffset
					+ (endValue.textSizeOffset - startValue.textSizeOffset) * fraction;
	...
	private static class GradientInfo {
		...
		public GradientInfo(float preFocusoffsetY, float postFocusoffsetY, float textSizeOffset, int textColor,
				int focusTextColor) {
			super();
		...
		public String toString() {
			return "GradientInfo [preFocusoffsetY=" + preFocusOffsetY + ", postFocusoffsetY=" + postFocusOffsetY
					+ ", textSizeOffset=" + textSizeOffset + ", textColor=" + textColor + ", focusTextColor="
					+ focusTextColor + "]";
		}
