...
public class FocusFrame extends AnimateView {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mState == State.MOVE) {
			switch (mDirection) {
			case HORIZONTAL:
				int toX = Math.round(progress * (mToXDelta - mFromXDelta)
						+ mFromXDelta);
				l = toX;
				t = mFromYDelta;
				r = toX + mWidth;
			...
			case VERTICAL:
			default:
				int toY = Math.round(progress * (mToYDelta - mFromYDelta)
						+ mFromYDelta);
				l = mFromXDelta;
				t = toY;
	...
	public void move(int fromCenterX, int fromCenterY, int toCenterX,
			int toCenterY) {
		if (mLastFromYDelta == fromCenterY && mLastToYDelta == toCenterY
				&& mLastFromXDelta == fromCenterX && mLastToXDelta == toCenterX)
			return;
		forceHide();
		mState = State.MOVE;
